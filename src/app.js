import { analyzePronunciation, getUnsupportedCases, runG2PTests } from "./g2p.js";
...
const elements = {
  ...
  unsupportedCaseButtons: document.querySelector("#unsupportedCaseButtons"),
...
function renderAnalysis(result) {
  ...
  if (result.unsupported) {
    setMessage(
      `현재 프로토타입에서는 미지원: ${result.unsupported.category} 규칙 보완이 필요합니다.`,
      "warning",
    );
  } else {
...
function renderUnsupportedCaseButtons() {
  const cases = getUnsupportedCases();
  elements.unsupportedCaseButtons.textContent = "";
  ...
  for (const item of cases) {
    ...
    button.type = "button";
    word.textContent = item.input;
    detail.textContent = `${item.expected} · ${item.category}`;

    button.append(word, detail);
    button.addEventListener("click", () => {
      elements.textInput.value = item.input;
      analyzeText(item.input);
    });

    elements.unsupportedCaseButtons.append(button);
  }
}
